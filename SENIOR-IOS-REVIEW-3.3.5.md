# MisMeeter 3.3.5 — Senior iOS review
...
3.3.5 separates receive-only and duplex audio-session policy. Receive-only uses `.playback`, which is the native category for audible playback and background/locked-screen audio. When TX joins an active RX session, MisMeeter switches to `.playAndRecord` with `.defaultToSpeaker`; when TX stops and RX remains active, it returns to `.playback`.

The receiver also observes route changes, session interruptions, and `AVAudioEngineConfigurationChange`. If iOS invalidates or stops the engine during a category/route transition, the output graph is prepared and restarted while the network/ring-buffer state remains alive. The mixer-to-output connection remains implicit so AVAudioEngine tracks the hardware output format across restarts.

## Control Center visual state

The Controls no longer attempt dynamic red/green tinting. Control Center primarily uses the template tint to represent the ON state, so 3.3.5 uses a stable green active tint and makes the state unambiguous with symbols and text:

- TX active: `mic.fill`, green; TX muted: `mic.slash.fill`, neutral; TX idle: disabled.
- RX active: `speaker.wave.3.fill`, green; RX muted: `speaker.slash.fill`, neutral; RX idle: disabled.
- `controlWidgetStatus` and `controlWidgetActionHint` provide explicit state/action language when Control Center has room to show it.

## Fixes after the 3.3.3 CI failure

The failing repository still contained old files that were automatically included because XcodeGen previously used recursive directory sources. 3.3.5 now enumerates every source file explicitly for both app and widget targets and regenerates `MisMeeter.xcodeproj` from scratch in CI.
...
3.3.5 replaces the single shared command slot with independent mailboxes for Mic, RX, and Stop All. Rapid interactions on different controls can no longer overwrite each other before the runtime consumes them. Stop All dominates a simultaneous command burst.
